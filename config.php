<?PHP
return array (
    'back_url' => 'http://lib.surgu.ru/',
    'mailTo' => 'vkr@surgu.ru',
    'work_type' => array('Дипломная работа', 'Магистерская диссертация', 'Бакалаврская работа'),
    'institute' => array(
    'Политехнический институт' => array(
        "11.04.02" => "Инфокоммуникационные технологии и системы связи (магистратура)",
        "11.03.02" => "Инфокоммуникационные технологии и системы связи(бакалавриат)",
        "09.04.01" => "Информатика и вычислительная техника (магистратура)",
        "09.03.01" => "Информатика и вычислительная техника(бакалавриат)",
        "09.04.02" => "Информационные системы и технологии (магистратура)",
        "09.03.02" => "Информационные системы и технологии(бакалавриат)",
        "01.03.02" => "Прикладная математика и информатика (бакалавриат)",
        "09.03.04" => "Программная инженерия(бакалавриат)",
        "08.03.01" => "Строительство(бакалавриат)",
        "27.04.04" => "Управление в технических системах (магистратура)",
        "27.03.04" => "Управление в технических системах(бакалавриат)",
        "03.04.02" => "Физика (магистратура)",
        "03.03.02" => "Физика(бакалавриат)",
        "13.04.02" => "Электроэнергетика и электротехника (магистратура)",
        "13.03.02" => "Электроэнергетика и электротехника(бакалавриат)"
                                ),
    'Институт гуманитарного образования и спорта' => array(
        "46.04.01" => "История (магистратура)",
        "46.03.01" => "История(бакалавриат)",
        "37.05.01" => "Клиническая психология (специалитет)",
        "45.04.02" => "Лингвистика (магистратура)",
        "45.03.02" => "Лингвистика(бакалавриат)",
        "51.03.04" => "Музеология и охрана объектов культурного и природного наследия(бакалавриат)",
        "51.03.02" => "Народная художественная культура(бакалавриат)",
        "44.04.01" => "Педагогическое образование (магистратура)",
        "44.03.01" => "Педагогическое образование(бакалавриат)",
        "44.03.04" => "Профессиональное обучение (бакалавриат)",
        "37.05.02" => "Психология служебной деятельности (специалитет)",
        "42.03.01" => "Реклама и связи с общественностью(бакалавриат)",
        "49.03.03" => "Рекреация и спортивно-оздоровительный туризм(бакалавриат)",
        "49.04.01" => "Физическая культура (магистратура)",
        "49.03.02" => "Физическая культура для лиц с отклонениями в состоянии здоровья(бакалавриат)",
        "49.04.02" => "Физическая культура для лиц с отклонкниями в состоянии здоровья (Адаптивная физическая культура) (магистратура)",
        "49.03.01" => "Физическая культура(бакалавриат)"
                                ),
    'Институт государства и права' => array(
        "41.04.04"	=> "Политология (магистратура)",
        "41.03.04" => "Политология(бакалавриат)",
        "40.04.01" => "Юриспруденция (магистратура)",
        "40.03.01" => "Юриспруденция(бакалавриат)"
                                ),
    'Институт естественных и технических наук' => array(
        "06.04.01" => "Биология (магистратура)",
        "06.03.01" => "Биология(бакалавриат)",
        "20.05.01" => "Пожарная безопасность (специалитет)",
        "20.03.01" => "Техносферная безопасность(бакалавриат)",
        "04.05.01" => "Фундаментальная и прикладная химия (специалитет)",
        "04.04.01" => "Химия (магистратура)",
        "04.03.01" => "Химия (бакалавриат)",
        "05.03.06" => "Экология и природопользование(бакалавриат)"
                                ),
    'Институт экономики и управления' => array(
        "38.04.04" => "Государственное и муниципальное управление (магистратура)",
        "38.03.04" => "Государственное и муниципальное управление(бакалавриат)",
        "38.04.02" => "Менеджмент (магистратура)",
        "38.03.02" => "Менеджмент(бакалавриат)",
        "38.04.03" => "Управление персоналом (магистратура)",
        "38.03.03" => "Управление персоналом(бакалавриат)",
        "38.04.08" => "Финансы и кредит  (магистратура)",
        "38.04.01" => "Экономика (магистратура)",
        "38.03.01" => "Экономика(бакалавриат)"
                                ),
    'Медицинский институт' => array(
        "31.05.01" => "Лечебное дело (специалитет)",
        "31.05.02" => "Педиатрия (специалитет)"
                                )
                        )
);